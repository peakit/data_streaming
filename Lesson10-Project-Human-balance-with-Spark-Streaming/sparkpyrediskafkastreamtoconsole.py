...
spark = SparkSession.builder.appName("redis-kafka-app").getOrCreate()
...
redisEncodedDF = redisRawDF.selectExpr("CAST(key as string) as key", "CAST(value as string) as value")
...
redisEncodedDF.withColumn("value", from_json("value", redisMessageSchema)).select(col("value.*")).createOrReplaceTempView("RedisSortedSet1")
...
customerEncodedDF = spark.sql("SELECT key, zSetEntries[0].element as Customer FROM RedisSortedSet1")
...
customerDecodedDF = customerEncodedDF.withColumn("Customer", unbase64(customerEncodedDF.Customer).cast("string"))
# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
customerJsonDF = customerDecodedDF.withColumn("Customer", from_json("Customer", customerSchema)).select(col("Customer.*")).createOrReplaceTempView("CustomerRecords1")
...
emailAndBirthDayStreamingDF = spark.sql("SELECT email, birthDay FROM CustomerRecords1 WHERE email is not NULL AND birthDay is not NULL")
