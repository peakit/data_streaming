...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "org.chicago.cta.station.table.v3": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic().startswith("org.chicago.cta.arrival.") and \
            message.topic().endswith(".v3"): # Set the conditional to the arrival topic
