...
def get_topic(topic_name):
    ...
    # TODO: See: https://docs.confluent.io/current/kafka-rest/api.html#get--topics
    resp = requests.get(f"{REST_PROXY_URL}/topics/com.udacity.streams.purchases")  # TODO

    try:
        resp.raise_for_status()
    except:
        print("Failed to get topic details {json.dumps(resp.json(), indent=2)})")

    print("Fetched topic details from Kafka:")
    print(json.dumps(resp.json(), indent=2))
...
def get_partitions(topic_name):
    ...
    try:
        ...
        print(f"Failed to get partitions details for {topic_name}")
        print(json.dumps(resp.json(), indent=2))
    print(json.dumps(resp.json(), indent=2))
