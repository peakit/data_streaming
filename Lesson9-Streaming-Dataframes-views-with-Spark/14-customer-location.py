...
spark = SparkSession.builder.appName("customer-location-app").getOrCreate()
...
redisEncodedStreamingDF = redisRawStreamingDF.selectExpr("CAST(key as string) as key", "CAST(value as string) as value")

#TO-DO: using the redisMessageSchema StructType, deserialize the JSON from the streaming dataframe 
redisEncodedStreamingDF.withColumn("value", from_json("value", redisMessageSchema)).select(col("value.*")).createOrReplaceTempView("RedisData")
...
customerLocationEncodedDF = spark.sql("SELECT key, zSetEntries[0].element as customerLocation FROM RedisData")
...
customerLocationDecodedDF = customerLocationEncodedDF.withColumn("customerLocation", unbase64(customerLocationEncodedDF.customerLocation).cast("string"))
...
customerLocationDecodedDF.withColumn("customerLocation", from_json("customerLocation", customer_location_schema)).select(col("customerLocation.*")).createOrReplaceTempView("CustomerLocation")

#TO-DO: using spark.sql select * from CustomerLocation
customerLocationFinalDF = spark.sql("SELECT * FROM CustomerLocation")
...
# |     93618942|Argentina|
# +-------------+---------+
customerLocationFinalDF.writeStream.outputMode("append").format("console").start().awaitTermination()
